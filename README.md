...
1. Download **fakepkg** script :

   `curl -s -o fakepkg https://gitlab.com/virtualdemon/archx/raw/master/fakepkg && chmod +x fakepkg`

2. Extract and install packages on /mnt : \[ Rebuilding live system installed packages \(with fakepkg\) \]

   `curl -s -o installer https://gitlab.com/virtualdemon/archx/raw/master/installer && chmod +x installer && ./installer`

3. You can read script's source before running it to see what will happen ... : `cat installer`  
   Also if you need to do your chroot customizations automatically, you can run following command in chroot environment :

   > if you were connected to internet in install duration it's already downloaded at '/mnt/tmp/auto\_chroot' so just run it : `/tmp/auto_chroot`

   `curl -s -o auto_chroot https://gitlab.com/virtualdemon/archx/raw/master/auto_chroot && chmod +x auto_chroot && ./auto_chroot`

4. When your customization finished you can `exit` and unmount devices with `umount -R /mnt` then `reboot` system to use your installed ArchLinux.

5. For using pacman \(if you didn't execute auto\_chroot\) as the first time run these commands :  
   `pacman -Sy`  
   `pacman-key --init`  
   `pacman-key --populate archlinux`  
   `pacman-key --refresh-keys`

6. To use the AUR packages just take a look at [trizen/trizen](https://github.com/trizen/trizen)
